#!/usr/bin/env python3
"""
Phase 5.2 -- The Wrapper (GEMSEO discipline around DAFoam)
==========================================================
A single GEMSEO Discipline that owns the OpenMDAO/MPhys problem
built in case/dafoam_model.py. Per evaluation it:

    1. pushes the FFD 'shape' vector and 'aoa' into the model
    2. warps the mesh (IDWarp, inside the model)
    3. runs the primal CFD                      -> CD, CL, geo constraints
    4. on demand, runs the discrete adjoint     -> exact Jacobians
    5. (optional) appends a summary line to `summary_path` after each primal

GEMSEO then sees an ordinary differentiable discipline and can drive
it with any of its gradient-based algorithms (SLSQP, PYOPTSPARSE_SLSQP,
NLOPT_SLSQP, ...).

Written for GEMSEO 6.x. If you downgrade to 5.x, `Discipline` must be
replaced with `MDODiscipline`, `io.data` with `local_data`,
`io.input_grammar.defaults` with `default_inputs`, and `_run(input_data)`
back to `_run(self)` + `self.store_local_data(**out)`.
"""
