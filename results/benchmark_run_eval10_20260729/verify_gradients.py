#!/usr/bin/env python3
"""
Phase 4.3 -- Gradient Verification (THE crucial step)
=====================================================
Proves the discrete adjoint before trusting it, two ways:

1. `--task check_totals` (default)
   OpenMDAO's check_totals: central finite differences on EVERY design
   variable vs. the adjoint totals. Exhaustive but costs
   2 x n_DV primal solves -- run on a coarse mesh.

2. `--task directional`
   Single random-direction dot-product test:

       FD:      (F(x + h d) - F(x - h d)) / (2h)
       Adjoint: dF/dx . d

   Two primal solves total, catches sign/scaling bugs immediately.
   Do this first, always.

Acceptance criterion (He et al. 2019 report the same order):
   relative error <= ~1e-4 for a tightly converged primal
   (primalMinResTol and gmresRelTol both drive this floor).

    mpirun -np 8 python verify_gradients.py --task directional
    mpirun -np 8 python verify_gradients.py --task check_totals
"""
import argparse
