...
Formal problem statement:
    ...
    subject to    CL(shape, aoa)  =  CL*      (lift equality)
                  ...
                  volcon          >= 1.0      (volume ratio)
                  rcon            >= 0.8      (LE radius ratio)

Gradients are supplied by the discrete adjoint of the compressible RANS
equations (via DAFoam) and delivered to GEMSEO through the user-supplied
Jacobian pathway (``set_differentiation_method("user")``). Each SLSQP
outer iteration costs one primal solve plus roughly one adjoint solve
per active functional, independent of the design-variable count.

The driver writes ``opt_summary.md`` alongside the run: a header with
all operating conditions and configuration, a per-evaluation table
(CD, CL, constraints, wall time), and a footer with the final result
and a paper reference comparison.
...
Must be launched from inside ``case/`` because OpenFOAM reads the mesh
from the current working directory:

    cd case
    mpirun -np 4 python ../mdo/run_optimization.py
    mpirun -np 4 python ../mdo/run_optimization.py \\
