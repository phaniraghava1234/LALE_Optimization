...
For the Spalart-Allmaras model resolved to the wall we target y+ ~ 1.
"""
...
# --- The two operating points used in this project -------------------------
...
if __name__ == "__main__":
    for name, fs in [("Tutorial (U=248, T=300, p=101325)", TUTORIAL),
                     ("Benchmark (M=0.729, Re=6.5e6)", BENCHMARK)]:
        r = first_cell_height(fs, y_plus=1.0)
        print(f"\n{name}")
        print(f"  M = {fs.M:.4f}, U = {fs.U:.2f} m/s, rho = {fs.rho:.4f} kg/m3, "
              f"p = {fs.p:.0f} Pa")
